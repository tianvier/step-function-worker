...
Task.prototype._fail = function (res) {
	...
	// }
	if (typeof res.error.name !== 'undefined') {
		error_name = res.error.name
	}
	let error_cause = JSON.stringify(res.error, replaceError);
	if (error_name.length > 256) {
		// Otherwise aws sdk will tell
		// failed to satisfy constraint: Member must have length less than or equal to 256
		error_name = error_name.slice(0, 253) + '...';
	}
	if (error_cause.length > 32768) {
		// Otherwise aws sdk will tell https://docs.aws.amazon.com/step-functions/latest/apireference/API_SendTaskFailure.html
		// failed to satisfy constraint: Member must have length less than or equal to 32768
		error_cause = error_cause.slice(0, 32765) + '...';
	}

	//const params = Object.assign({}, res, {error});
	const params = Object.assign({}, res, { error: error_name, cause: error_cause });
	delete params.workerName;
	delete params.input;
